...
export class SignInUp {
  ...
  register(): void {
    ...
    if (uName.length < 3) {
      this.toastr.error(
        "Benutzername muss mindestens 3 Buchstaben lang sein!",
        "Fehler"
      );
      return;
    }
    if (uEmail.length < 3 || !uEmail.includes("@")) {
      this.toastr.error(
        "Bitte geben Sie eine vollständige Email ein!",
        "Fehler"
      );
      return;
    }
    ...
    this.auth.register(uName, uEmail, uPassword).subscribe({
      ...
      error: () => {
        this.toastr.error("Benutzer existiert bereits!", "Fehler");
      },
    });
  }
  ...
  login(): void {
    ...
    this.auth.login(uNameEmail, uPassword).subscribe({
      ...
      error: () => {
        this.toastr.error(
          "Benutzername / Email oder Passwort ist inkorrekt!",
          "Fehler"
        );
      },
    });
